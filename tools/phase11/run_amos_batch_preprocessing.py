#!/usr/bin/env python3
"""
tools/phase11/run_amos_batch_preprocessing.py

High-performance batch processing for AMOS CT (and MRI) cases:
- Canonical RAS reorientation
- Intensity-only surface extraction (HU > -500 for CT)
...
- Visual QC panels for 20 CT cases
...
def extract_ct_body_mask(volume_hu):
    ...
    # Axial slice-by-slice hole filling
...
def process_case_task(args):
    ...
    try:
        ...
        # Marching cubes with uint8 optimization
        ...
        if lbl_nii is not None:
            ...
            for _, row in target_mapping.iterrows():
                ...
                if len(vox_coords) >= 10:
                    organ_centers_vox[amos_id] = np.mean(vox_coords, axis=0)
        ...
        if do_qc:
            ...
            axes[0].contour(np.rot90(body_mask[:, :, mid_z]), levels=[0.5], colors="red", linewidths=1.5)
            axes[0].set_title(f"{case_id} Axial (z={mid_z})")
            axes[0].axis("off")

            # Coronal slice
            axes[1].imshow(np.rot90(img_data[:, mid_y, :]), cmap="gray")
            axes[1].contour(np.rot90(body_mask[:, mid_y, :]), levels=[0.5], colors="red", linewidths=1.5)
            axes[1].set_title(f"{case_id} Coronal (y={mid_y})")
            axes[1].axis("off")

            # 3D surface points overlay
...
def main():
    print("=== AMOS High-Performance Batch Preprocessing ===")
    img_files = {os.path.basename(f): f for f in glob.glob("data_external/AMOS22/extracted/imagesTr/amos_*.nii.gz")}
    lbl_files = {os.path.basename(f): f for f in glob.glob("data_external/AMOS22/extracted/labelsTr/amos_*.nii.gz")}

    paired_names = sorted(list(set(img_files.keys()).intersection(set(lbl_files.keys()))))
    print(f"Discovered {len(paired_names)} paired cases in imagesTr / labelsTr.")

    tasks = []
    qc_count = 0
    for bname in paired_names:
        cid = bname.split(".")[0]
        cnum = int(cid.split("_")[1])
        modality = "CT" if cnum <= 500 else "MRI"
        do_qc = (qc_count < 20)
        if do_qc:
            qc_count += 1
        tasks.append((img_files[bname], lbl_files[bname], cid, modality, do_qc))

    all_gt = []
    all_fov = []
    t0 = time.time()
    print(f"Launching batch execution across {len(tasks)} cases with 8 parallel workers...")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        ...
        for f in concurrent.futures.as_completed(futures):
            ...
            if completed % 25 == 0 or completed == len(tasks):
                print(f"  Processed {completed}/{len(tasks)} cases ({time.time()-t0:.1f}s elapsed)...")

    # Save outputs
    df_gt = pd.DataFrame(all_gt)
    df_gt.to_csv(GT_CENTROIDS_PATH, index=False)
    print(f"Saved {len(df_gt)} GT centroid records to {GT_CENTROIDS_PATH}")

    df_fov = pd.DataFrame(all_fov)
    df_fov.to_csv(FOV_AUDIT_PATH, index=False)
    print(f"Saved {len(df_fov)} FOV audit records to {FOV_AUDIT_PATH}")
    print(f"FOV categories breakdown:\n{df_fov['fov_category'].value_counts()}")
