...
def run_epoch_train(
    ...
    for step, batch in pbar:
        ...
        with autocast(device_type=device.type,enabled=cfg.mixed_precision):
            ...
            loss, _, _ = criterion(pred_hm, gt_hm, gt_ctr, mask)
...
def main():
    ...
    n_train = int(cfg.train_val_split * len(full_ds))
    n_val   = len(full_ds) - n_train
    train_ds, val_ds = random_split(
        full_ds, [n_train, n_val],
        generator=torch.Generator().manual_seed(cfg.seed),
    )
    # Enable augmentation only on train subset
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        print(
            ...
            f"AvgErr={avg_err:.2f}mm | LR={cur_lr:.2e} | {elapsed:.0f}s"
        )
        for name, err in per_organ.items():
            print(f"  {name:<15} {err:.2f} mm")
        ...
        if writer:
            ...
            for name, err in per_organ.items():
                writer.add_scalar(f"Organs/{name}_mm", err, epoch)
 
        # ── CSV ───────────────────────────────────────────────
        if cfg.log_csv:
            row = [epoch + 1, round(train_loss, 5), round(val_loss, 5),
                   round(avg_err, 3), f"{cur_lr:.2e}"]
            row += [round(per_organ[n], 3) for n in ORGAN_NAMES]
            append_csv(csv_path, row)
 
        # ── Checkpointing ─────────────────────────────────────
        state = build_state(epoch, model, optimizer, scheduler, scaler,
                            best_val_error, dataclasses.asdict(cfg))
 
        # 1) latest_model.pth – every epoch
        save_checkpoint(state, cfg.checkpoint_dir, "latest_model.pth")
 
        # 2) best_model.pth – when validation error improves
        ...
        # 3) checkpoint_epoch_X.pth – every N epochs
