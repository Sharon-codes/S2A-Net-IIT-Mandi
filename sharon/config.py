...
class Config:
    ...
    backbone: str = "gnn"           # gnn | swin_unetr | densenet121
    ...
    centroid_weight: float = 20.0

    # Scheduler
    scheduler: str = "cosine"       # cosine | plateau | step
    warmup_epochs: int = 2
    lr_min: float = 1e-6
    patience: int = 150             # for plateau / early stopping
    ...
    def __post_init__(self):
        self.checkpoint_dir = str(Path(self.output_dir) / "checkpoints" / self.backbone)
        self.log_dir = str(Path(self.output_dir) / "logs" / self.backbone)
        self.tensorboard_dir = str(Path(self.output_dir) / "tensorboard" / self.backbone)
        self.predictions_dir = str(Path(self.output_dir) / "predictions" / self.backbone)
    ...
    def from_args(cls) -> "Config":
        parser = argparse.ArgumentParser(description="CT 117-Organ Localization Training")
        parser.add_argument("--config", type=str, default=None, help="Path to YAML config")
        parser.add_argument("--data_root", type=str)
        parser.add_argument("--output_dir", type=str)
        parser.add_argument("--backbone", type=str, default=None, help="Backbone encoder architecture")
        parser.add_argument("--epochs", type=int)
        parser.add_argument("--batch_size", type=int)
        parser.add_argument("--learning_rate", type=float)
        parser.add_argument("--num_workers", type=int)
        parser.add_argument("--grad_accum_steps", type=int)
        parser.add_argument("--mixed_precision", type=str, default="True", help="Mixed precision True/False")
        ...
        if args.mixed_precision is not None:
            cfg.mixed_precision = str(args.mixed_precision).lower() in ("true", "1", "yes")
